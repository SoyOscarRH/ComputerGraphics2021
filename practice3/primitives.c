        ...
        *numElements = p2.x - p1.x;
        ...
        if(m > 0 && m < 1){
            *numElements = 1+p2.x - p1.x;
            ...
            Point2DInt *transformedPoints = (Point2DInt*)calloc(1+p2t.x - p1t.x, sizeof(Point2DInt));
            int counter = 0;
            for(int i = p1t.x; i <= p2t.x; i++){
                transformedPoints[counter].x = i;
                transformedPoints[counter].y = (int)round(mTransformed*i+bTransformed);
                counter++;            
            }
...
void parallelNaiveLineDrawing(PixelRGB **image, PixelRGB *color, Point2DInt p1, Point2DInt p2, int numThreads, ThreadPool *pool){
    ...
    float realX = p1.x;
    float realY = p1.y;
    InfoParallelLineDrawing *infoThread=(InfoParallelLineDrawing*)calloc(numThreads, sizeof(InfoParallelLineDrawing));
    //printf("Calculando linea...\n");
    for(int i=0; i < numThreads; i++){
        //printf("%d: %d -> %d (%f)\n", i, (int)realX, (int)(realX+xInc), realX+xInc);
        //printf("%d: %d -> %d (%f)\n", i, (int)realY, (int)(realY+yInc), realY+yInc);
        //printf("\n");
...
void drawFace(PixelRGB **image, PixelRGB color, FaceNode face, Point2DInt *vertices, int lineAlgorithm){
    ...
    switch(lineAlgorithm){
        case 0:
            while(aux->next){
                linePoints = naiveLineDrawing(vertices[aux->value-1], vertices[aux->next->value-1], &numPoints);
                setLine(linePoints, numPoints, image, color);
                aux = aux->next;
                free(linePoints);
            }
            linePoints = naiveLineDrawing(vertices[aux->value-1], vertices[face.face->value-1], &numPoints);
            setLine(linePoints, numPoints, image, color);
            free(linePoints);
        break;
        ...
        case 1:
            while(aux->next){
                linePoints = DDALineDrawing(vertices[aux->value-1], vertices[aux->next->value-1], &numPoints);
                setLine(linePoints, numPoints, image, color);                
                aux = aux->next;
                free(linePoints);
            }
            linePoints = DDALineDrawing(vertices[aux->value-1], vertices[face.face->value-1], &numPoints);
            setLine(linePoints, numPoints, image, color);
            free(linePoints);
        break;
        ...
        case 2:
            while(aux->next){
                linePoints = BresenhamLineDrawing(vertices[aux->value-1], vertices[aux->next->value-1], &numPoints);
                setLine(linePoints, numPoints, image, color);
                aux = aux->next;
                free(linePoints);
            }
            linePoints = BresenhamLineDrawing(vertices[aux->value-1], vertices[face.face->value-1], &numPoints);
            setLine(linePoints, numPoints, image, color);
            free(linePoints);
        break;
    }
...
void drawFaceParallel(PixelRGB **image, PixelRGB *color, FaceNode face, Point2DInt *vertices, int lineAlgorithm, ThreadPool *pool, int numThreads){
    ...
    switch(lineAlgorithm){
        case 0:
            ...
            while(aux->next){
                parallelNaiveLineDrawing(image, color, vertices[aux->value-1], vertices[aux->next->value-1], numThreads, pool);
                aux = aux->next;
            }
            parallelNaiveLineDrawing(image, color, vertices[aux->value-1], vertices[face.face->value-1], numThreads, pool);
        break;
    }
}
